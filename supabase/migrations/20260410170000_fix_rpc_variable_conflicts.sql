-- Fix ambiguous column references in RPCs whose RETURNS TABLE output
-- columns share names with underlying table columns (PL/pgSQL implicitly
-- declares OUT variables for each output column, which then shadow the
-- real table columns inside statements like ON CONFLICT and bare UPDATE
-- targets). `#variable_conflict use_column` tells PL/pgSQL to prefer the
-- table column when there's a conflict.
